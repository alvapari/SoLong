...
/*   Updated: 2024/10/06 21:17:23 by alvapari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_add_step(t_data *data, int old_plx_pos, int old_ply_pos, char key)
{
	...
	ft_print_steps(data);
	if ((data->map.mtrx[new_y][new_x] == 'E' && (data->cnt_colls)
		== (data->max_colls * 2)) || data->map.mtrx[new_y][new_x] == 'N')
		ft_win_and_lose_management(data, new_y, new_x);
}

void	ft_win_and_lose_management(t_data *data, int new_y, int new_x)
{
	if (data->map.mtrx[new_y][new_x] == 'N')
	{
...
void	ft_change_player_position(t_data *data, char key)
{
	...
	if (data->steps > 0)
		mlx_delete_image(data->mlx, data->images.steps_on_screen);
}
